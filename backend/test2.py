...
# # Create a named window and a trackbar
# cv2.namedWindow('image')
...
#         mask = np.zeros(img.shape[:2], dtype=np.uint8)
#         mask = cv2.fillConvexPoly(mask, np.array([shape[i] for i in left], dtype=np.int32), 255)
#         mask = cv2.fillConvexPoly(mask, np.array([shape[i] for i in right], dtype=np.int32), 255)
#         mask = cv2.dilate(mask, kernel, 5)
...
#         # Contouring function for detecting eyes
#         def contouring(thresh, mid, img, right=False):
#             cnts, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
#             try:
#                 cnt = max(cnts, key=cv2.contourArea)
...
while True:
    ...
    # Initialize thresh to avoid errors if no face is detected
